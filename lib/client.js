// The browser half of dsh-labrador: the dog himself.
//
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** How much he squashes when poked, and for how long. */
    ...
    /** Below this speed on the ground, he has stopped rather than landed. */
     ...
     *
     * `actAfterMs` is how long he will stand doing nothing before choosing
     * something to do; `sleepAfterMs` is how long he stays awake after being
    ...
    /**
     * The pose he holds while doing nothing.
     *
     * Without one he would be invisible: idle is a resting state, not an absence,
    ...
    /**
     * The pose he holds while asleep, preferring an action that declares itself one.
     * @param {Array<any>} actions @param {() => number} rng @param {any} temperament
     */
    ...
    /** The frame the dog is showing, or null when he is between poses. */
     ...
     * resting pose is chosen immediately, so he is never briefly invisible.
    ...
    function step(state, ctx, event) {
      ...
      // A drag is never interrupted by anything. He keeps whatever pose he was
      ...
      switch (event.type) {
        case 'landed': {
          // The component owns the physics; it tells us when he has stopped.
          if (state.phase !== 'thrown') return state;
          return toIdle(state, rng, temperament, actions);
        }

        case 'bounce': {
          // Hitting something mid-flight changes his face. He stays in the air —
          ...
          // His flight time is preserved across the impact: beginAction would reset
        ...
        case 'dragEnd': {
          ...
          // He keeps his pose while flying: the physics settles him on landing.
        ...
        case 'tick': {
          ...
          switch (state.phase) {
            ...
            case 'acting': {
              ...
              // 'hold' and 'once' release him back to idle to choose again.
            ...
            case 'thrown':
              // The physics tells us when he has landed; this is only a backstop
              // so a lost animation frame can never strand him mid-air.
    ...
    /**
     * Place the dog's interactive area over the part of the canvas he occupies.
     *
     * Without this a click anywhere in the transparent margin would count as
     * touching him, and he would swallow clicks meant for the app behind him.
    ...
    function hitStyle(hit, canvas, scale = 1) {
      if (hit === null || hit === undefined) {
        ...
        // him untouchable. Slightly greedy beats impossible to pet.
     ...
     *
     * Mutates `vel`, which is the caller's live velocity, and reports whether he
    ...
    function integrate(pos, vel, dt, bounds) {
      ...
      // gravity would push him into the floor every frame and he would "bounce"
    ...
    /**
     * The stage: the dog, deciding for himself what to do, and putting up with
    ...
    function LabradorStage({ temperament: initialTemperament = 'attentive' } = {}) {
      ...
      const action = actions.find((a) => a.id === state.actionId) ?? null;
      const frame = action?.frames?.[state.frameIndex] ?? null;
      ...
      /** Where he sits when nobody has moved him, clamped into the window. */
      ...
      // Changing the pet's size can leave him hanging off the edge of the window,
      // and once lost from view he cannot be dragged back. Pull him in.
      React.useEffect(() => {
        if (pos === null) return;
        const vw = typeof window.innerWidth === 'number' ? window.innerWidth : 1280;
        const vh = typeof window.innerHeight === 'number' ? window.innerHeight : 800;
      ...
      /** Move him, keeping him wholly inside the window. */
      ...
      // A spoken sound fades on its own; he does not hold a thought for long.
      React.useEffect(() => {
      ...
      // Occasionally he mutters to himself when he settles on something to do,
      ...
      const onPointerDown = (event) => {
        ...
        // Touching him means the menu is no longer wanted.
        setMenu(null);
      ...
      // While he is in the air, the physics runs.
      React.useEffect(() => {
        ...
        const loop = (now) => {
          ...
          if (next.bounced) {
            // Every impact swaps his face, so a throw looks like a scramble
      ...
      const stage = {
        ...
        // below opts back in, so the margin around him remains click-through.
        pointerEvents: 'none',
      ...
      return React.createElement(
        ...
        // A bark above his head: vocalisations only, never words.
        bubble !== null
          ? React.createElement('div', {
